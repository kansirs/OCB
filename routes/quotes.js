...
router.post('/quote', quoteLimiter, (req, res) => {
  ...
  const budgetRange = (body.budget_range || '').trim();
  ...
  const stmt = db.prepare(`
    INSERT INTO leads (name, phone, email, address, service, budget_range, message, ip_address)
    VALUES (@name, @phone, @email, @address, @service, @budget_range, @message, @ip_address)
  `);
